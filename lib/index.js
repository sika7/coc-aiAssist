...
function getConfig() {
  const config = import_coc.workspace.getConfiguration("claude");
  return {
    apiKey: config.get("apiKey", ""),
    model: config.get("module", "claude-3-7-sonnet-20250219"),
    maxTokens: config.get("maxTokens", 1e3)
  };
}
...
var Logger = class {
  constructor(name) {
    this.channel = import_coc2.window.createOutputChannel(name);
  }
  info(message) {
...
function removeMarkdownFormat(text) {
...
async function yankText(text) {
  await import_coc3.workspace.nvim.call("setreg", ["*", text]);
...
async function selectedText() {
  ...
  const range = await import_coc3.window.getSelectedRange("");
  let context = "";
...
function makePrompt(question, code) {
  let prompt = question;
  if (code && code.trim() !== "") {
    prompt = `${question}
...
var ClaudeClient = class {
  ...
  async sendMessage(message) {
    try {
      ...
      const system = isCodeOnlyRequest ? "You are a code generator. You always respond with only functioning code, no explanations outside of code comments." : "You are a helpful assistant for developers. Provide clear, concise responses that can be directly used in code.";
      const requestData = {
        model: this.model,
...
async function focusWinId(winId) {
  await import_coc4.workspace.nvim.call("win_gotoid", [winId]);
}
async function showQuestionBox(placeholder = "") {
  const input = await import_coc4.window.createInputBox("\u8CEA\u554F", "", {
    minWidth: 80,
    position: "center",
    placeholder
  });
...
// src/index.ts
function createCodeOnlyPrompt(question) {
...
async function activate(context) {
  logger.info("Claude Command extension activated");
  ...
  const log = import_coc5.commands.registerCommand("claude.log", async () => {
    logger.show();
  });
  const cmd = async (args = {
    ...
    try {
      ...
      if (!question) {
        ...
        import_coc5.window.showInformationMessage(
          "Empty question, not sending request to API"
        );
        return;
      }
      const context2 = await selectedText();
      let prompt = makePrompt(question, context2);
      if (codeOnlyMode)
        prompt = createCodeOnlyPrompt(prompt);
      logger.info(prompt);
  ...
  const ask = import_coc5.commands.registerCommand("claude.ask", async () => {
  ...
  const codeOnly = import_coc5.commands.registerCommand("claude.askCodeOnly", async () => {
  ...
  const closeDialogCommandDisposable = import_coc5.commands.registerCommand(
    "claude.closeDialog",
    async () => {
    }
  );
  const focusDialogCommandDisposable = import_coc5.commands.registerCommand(
    "claude.focusDialog",
    async () => {
    }
  );
  const visualAsk = import_coc5.workspace.registerKeymap(
    ["v"],
    "claude-ask",
    async () => {
      await cmd({
  ...
  const visualCodeOnly = import_coc5.workspace.registerKeymap(
    ["v"],
