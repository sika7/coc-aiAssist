...
var Logger = class {
  constructor(name) {
    this.channel = import_coc.window.createOutputChannel(name);
  }
  info(message, data) {
...
async function postRequest(url2, data, headers) {
  try {
...
var EventManager = class {
  ...
  registerCallback(name, callback) {
    if (this.callbacks.find((item) => item.name === name))
      return;
    this.callbacks.push({
      name,
      callback
...
async function activate(context) {
  ...
  const test2 = import_coc5.commands.registerCommand("aiAssist.test", async () => {
